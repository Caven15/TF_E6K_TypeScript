...
console.log("--- Exo 2 ---");
function classerNombre(nombre: number): string {
    if (nombre > 0) {
        return "Positif";
    } else if (nombre < 0) {
        return "Négatif";
...
console.log("--- Exo 3 ---");
function estPair(nombre:number):string {
    if (nombre % 2 == 0) {
        return "paire"
    } else {
        return "impaire"
...
let mdp : string = "azertyu8"
function verifierMotDePasse(mdp : string) : string{
    let retour : string = "Mot de passe trop court..."
    if(mdp.length >= 8){
...
console.log("--- Exo 6 ---")
function evaluerNote(note:number){
    if(note >= 0 && note <= 100){
        if(note > 89){
            return ("A")
        }
...
console.log("--- Exo 8 ---")
function analyserScores(scores : number[]) : {max : number, min : number, moyenne : number , mediane : number}{
    ...
    const moyenne : number = somme / scores.length
...
// Exemple 
const newTabScores : number[] = [70,60,96,75,85]
